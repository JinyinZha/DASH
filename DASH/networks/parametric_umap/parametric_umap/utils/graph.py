...
class TorchL2Index:
    def __init__(self, dim, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        self.device = device
        self.data = None
        self.dim = dim
    def add(self, x):
        ...
        if self.data is None:
            ...
            self.data = torch.cat((self.data, x_tensor))
    def search(self, queries, k):
        q_tensor = torch.tensor(queries).to(self.device)
        # 高效矩阵运算
        dist = torch.cdist(q_tensor, self.data, p=2)**2
        distances, indices = torch.topk(dist, k=k, dim=1, largest=False)
...
def compute_sigma_i(
    ...
    index = TorchL2Index(dim=n_features)
    index.add(X)
