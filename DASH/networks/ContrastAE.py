...
class Contrast_AE(nn.Module):
    ...
    def get_loss(self,this_ids):
        ...
        x1 = torch.from_numpy(self.xs[x1_ids]).to(self.device)
        x2 = torch.from_numpy(self.xs[x2_ids]).to(self.device)
        xs = torch.from_numpy(self.xs[unq_ids]).to(self.device)
        y_ref = torch.from_numpy(self.rmsds[this_ids].reshape(-1,1)).to(self.device)
        y,x_reconst = self.forward(x1,x2,xs)
