...
from einops.einops import EinopsError, repeat,_apply_recipe, _prepare_transformation_recipe
...
def safe_div(num, den, eps:float = 1e-8):
    res = num.div(den.clamp(min = eps))
    res.masked_fill_(den == 0, 0.)
    return res

def batched_index_select(values, indices, dim: int = 1):
    value_dims = values.shape[(dim + 1):]
    #values_shape, indices_shape = map(lambda t: list(t.shape), (values, indices))
    values_shape = list(values.shape)
    indices_shape = list(indices.shape)
    #indices = indices[(..., *((None,) * len(value_dims)))]
    for _ in range(len(value_dims)):
        indices = indices.unsqueeze(-1)

    #indices = indices.expand(*((-1,) * len(indices_shape)), *value_dims)
    expand_shape = list(indices_shape)
    for dim_size in value_dims:
        expand_shape.append(dim_size)
    indices = indices.expand(expand_shape)

    value_expand_len = len(indices_shape) - (dim + 1)
    
    #values = values[(*((slice(None),) * dim), *((None,) * value_expand_len), ...)]
    for i in range(int(value_expand_len)):
        values = values.unsqueeze(dim + 1)

    value_expand_shape = [-1] * len(values.shape)
    #expand_slice = slice(dim, (dim + value_expand_len))
    for i in range(int(dim),int(dim + value_expand_len)):
        value_expand_shape[i] = indices.shape[i]
...
def fourier_encode_dist(x, num_encodings: int = 4, include_self:bool = True):
    x = x.unsqueeze(-1)
...
class CoorsNorm(nn.Module):
    def __init__(self, eps:float = 1e-8, scale_init:float = 1.):
        super().__init__()
        self.eps = eps
...
class Attention(nn.Module):
    ...
    def forward(self, x, context, mask = None):
        ...
        q, k, v = map(lambda t: reduce(t, 'b n (h d) -> b h n d',  "rearrange",{"h":h}),(q, *kv))
        dots = einsum('b h i d, b h j d -> b h i j', q, k) * self.scale

        if mask is not None:
            mask_value = -torch.finfo(dots.dtype).max
            mask = reduce(mask, 'b n -> b () () n',"rearrange")
            dots.masked_fill_(~mask, mask_value)

        attn = dots.softmax(dim = -1)
        out = einsum('b h i j, b h j d -> b h i d', attn, v)

        out = reduce(out, 'b h n d -> b n (h d)',"rearrange", {"h":h})
...
class EGNN(nn.Module):
    ...
    def forward(self, feats, coors, edges = None, mask = None, adj_mat = None):
        b, n, d, device, fourier_features, num_nearest, valid_radius, only_sparse_neighbors = feats.shape[0], feats.shape[1], feats.shape[2], feats.device, self.fourier_features, self.num_nearest_neighbors, self.valid_radius, self.only_sparse_neighbors
        ...
        if use_nearest:
            ...
            if adj_mat is not None:
                if len(adj_mat.shape) == 2:
                    adj_mat = adj_mat.clone().unsqueeze(0).expand(b, -1, -1)
                ...
                adj_mat = adj_mat.masked_fill(self_mask, 0)
        ...
        if self.coors_mlp is not None:
            ...
            coor_weights = coor_weights.squeeze(-1)
            ...
            # 替换 einsum 操作
            # 原始: coors_out = einsum('b i j, b i j c -> b i c', coor_weights, rel_coors) + coors
            # 等价实现:
            # 1. 将 coor_weights 扩展到与 rel_coors 相同的维度
            coor_weights_expanded = coor_weights.unsqueeze(-1)  # [b, i, j, 1]
            # 2. 加权求和
            weighted_rel_coors = coor_weights_expanded * rel_coors  # [b, i, j, c]
            # 3. 沿 j 维度求和
            coors_out = weighted_rel_coors.sum(dim=-2) + coors  # [b, i, c]
...
class EGNN_Network(nn.Module):
    ...
    def forward(
        ...
        if exists(self.pos_emb):
            ...
            feats += reduce(pos_emb, 'n d -> () n d',"rearrange")
