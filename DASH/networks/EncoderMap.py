...
class EncoderMap(nn.Module):
    ...
    def __init__(self,xs,info,outdir,sm_loss_ratio=100,high_dim_params=None,low_dim_params=None,device="cuda"):
        ...
        self.info = info
        self.outdir = outdir
        self.sm_loss_ratio = sm_loss_ratio
    ...
    def get_loss(self,choice,use_all = False):
        ...
        sketch_loss = 1/n_dist * torch.sum( ( self.sigmoid(dm_h, **self.high_dim_params ) - self.sigmoid(dm_l, **self.low_dim_params ) )**2 )
        return nn.MSELoss()(y,x) + self.sm_loss_ratio * sketch_loss, y
    ...
    def train(self,random_seed):
        ...
        for n in range(epoch):
            #for xnp in xs_split: 
            for i in range(400):
                loss = self.get_loss(np.random.choice(len(self.xs),1000,replace=False))[0]
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            loss,x_reconst = self.get_loss(-1,True)
